...
CREATE OR REPLACE FUNCTION public.close_time_bank_month(_employee_id uuid, _year integer, _month integer, _decision text, _paid_minutes integer DEFAULT 0, _notes text DEFAULT NULL::text, _attendance_debit_minutes integer DEFAULT 0, _force boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  _closure_id uuid;
  _pending_candidates int := 0;
  _effective_notes text;
BEGIN
  ...
  -- Trava: candidatos pendentes de aprovação no mês
  SELECT COUNT(*) INTO _pending_candidates
    FROM public.overtime_approvals
   WHERE employee_id = _employee_id
     AND status = 'pending'
     AND record_date >= _first_day
     AND record_date < (_first_day + interval '1 month');

  IF _pending_candidates > 0 AND NOT COALESCE(_force, false) THEN
    RAISE EXCEPTION 'Existem % candidato(s) de aprovação pendente(s) neste mês. Aprove ou rejeite-os antes de fechar, ou force o fecho conscientemente.', _pending_candidates;
  END IF;

  _effective_notes := _notes;
  IF _pending_candidates > 0 AND COALESCE(_force, false) THEN
    _effective_notes := COALESCE(_notes,'') || ' [Fechado com ' || _pending_candidates || ' candidatos pendentes — forçado]';
  END IF;
  ...
  INSERT INTO public.time_bank_monthly_closures (
    ...
    _closing, _closing,
    _decision, _effective_notes, _payout_id,
    _uid, now(), true
  )
  ...
  RETURN jsonb_build_object(
    ...
    'pending_candidates', _pending_candidates,
    'forced', COALESCE(_force,false) AND _pending_candidates > 0
  );
END;
$function$;
